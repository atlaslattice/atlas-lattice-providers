...
class ProjectOrientedFeaturesEngine:
    ...
    async def _run_project_memory_graph(self, query: str, **kwargs) -> Dict[str, Any]:
        ...
        # MVP graph: simple dict search + delegate to notion rag if available (safe guard for dict/wrapper cases in symbiosis)
        if self.notion_engine and hasattr(self.notion_engine, "run") and not isinstance(self.notion_engine, dict):
            try:
    ...
    async def run(self, feature: str, **kwargs) -> Dict[str, Any]:
        ...
        # Normalize to _ (registry uses underscores); support - and spaces too
        ...
        # Priority 5: enforce mandatory human gate for high-stakes features even on direct project calls (symbiotic with orchestrator)
        high_stakes_keys = {"bullshit_olympics", "self_improving_skills", "arena_mode", "debate_arena", "physical", "immutable_ledger_replay", "narrative_coherence"}
        if key in high_stakes_keys or kwargs.get("high_stakes"):
            if self.copilot_engine:
                try:
                    card_payload = {"type": "AdaptiveCard", "body": [{"type": "TextBlock", "text": f"E145 high-stakes promotion gate for {key}"}], "actions": [{"type": "Action.Submit", "title": "PROMOTE"}]}
                    gate = await self.copilot_engine.run("teams_adaptive_cards", team_id="lattice-e145", channel_id="project-gates", card_json=card_payload)
                    result["mandatory_human_gate"] = gate
                    result["gate_status"] = "PENDING_APPROVAL_SIM" if self.simulate else "POSTED"
                except Exception as ge:
                    result["mandatory_human_gate_error"] = str(ge)
        return result
